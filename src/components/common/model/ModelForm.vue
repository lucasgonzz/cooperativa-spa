...
export default {
	...
	methods: {
		...
		getCol(prop, size) {
			if (this.input_full_width || this.useSearch(prop) || prop.has_many || (prop.belongs_to_many && prop.belongs_to_many.can_not_modify)) {
				return 12
			} 
			return size
		},
					...
					.then(res => {
						...
						if (this.has_many_parent_model) {
							...
							if (this.model_name == 'user') {
								this.$store.commit('auth/add', res.data.model)
							} else {
